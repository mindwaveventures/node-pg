...
const updateUserController = async (req, res, next) => {
  const userData = "select * from account_users au where user_id = $1";
  ...
  if (searchPgRes.rowCount == 0) {
    const queryText =
      "update account_users set first_name  = $1, last_name=$2, email=$3, user_name=$4, user_password=$5, phone_no=$6 where user_id = $7  returning *";
    const pgRes = await pgClient.query(queryText, [
...
const getAccountController = async (req, res, next) => {
  const queryText = "select * from account_users au where user_id = $1";
